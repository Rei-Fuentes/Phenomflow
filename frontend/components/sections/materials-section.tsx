...
export function MaterialsSection({ onAnalyze, isLoading, t }: MaterialsSectionProps) {
    ...
    const handleTranscriptionComplete = (transcriptions: { filename: string; text: string }[]) => {
        // Combine all transcriptions into one text
        const combinedText = transcriptions.map((t, i) =>
            `\n${'='.repeat(80)}\nFILE ${i + 1}: ${t.filename}\n${'='.repeat(80)}\n\n${t.text}\n`
        ).join('\n')

        setInputText(combinedText)
        setActiveTab("documents") // Switch to documents tab to show the transcription
    }
            ...
            <div className="max-w-4xl mx-auto w-full">
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <TabsContent value="documents" className="space-y-8">
                        ...
                        <div className={`flex justify-end transition-all duration-700 delay-300 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
                            <MagneticButton
                                size="lg"
                                variant="primary"
